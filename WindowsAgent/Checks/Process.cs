        ...
        public override CheckResult Run()
        {
            ...
            foreach (KeyValuePair<string, AggrItem> instance in aggr)
            {
                var item = new Item
                {
                    ...
                    ["ElapsedTime"] = (long)instance.Value["Elapsed Time"].Sum(),
                    ["HandleCount"] = (long)instance.Value["Handle Count"].Sum(),
                    ["IDProcess"] = instance.Value["ID Process"].ConvertAll(a => (int)a),
                    ["PageFaultsPersec"] = instance.Value["Page Faults/sec"].Sum(),
                    ["PageFileBytes"] = (long)instance.Value["Page File Bytes"].Sum(),
                    ["PageFileBytesPeak"] = (long)instance.Value["Page File Bytes Peak"].Max(),
                    ...
                    ["PoolNonpagedBytes"] = (long)instance.Value["Pool Nonpaged Bytes"].Sum(),
                    ["PoolPagedBytes"] = (long)instance.Value["Pool Paged Bytes"].Sum(),
                    ["PriorityBase"] = instance.Value["Priority Base"].ConvertAll(a => (int)a),
                    ["PrivateBytes"] = (long)instance.Value["Private Bytes"].Sum(),
                    ["ProcessCount"] = instance.Value["Creating Process ID"].Count(),
                    ["ThreadCount"] = (long)instance.Value["Thread Count"].Sum(),
                    ["VirtualBytes"] = (long)instance.Value["Virtual Bytes"].Sum(),
                    ["VirtualBytesPeak"] = (long)instance.Value["Virtual Bytes Peak"].Max(),
                    ["WorkingSet"] = (long)instance.Value["Working Set"].Sum(),
                    ["WorkingSetPeak"] = (long)instance.Value["Working Set Peak"].Max(),
                };
                items[index++] = item;
            };
