        ...
        public override CheckResult Run()
        {
            ...
            foreach (var instance in _counterCache)
            {
                var item = new Item
                {
                    ...
                    ["CurrentBandwidth"] = Convert.ToUInt32(instance.Value["Current Bandwidth"].NextValue()),
                    ["PacketsOutboundDiscarded"] = Convert.ToUInt32(instance.Value["Packets Outbound Discarded"].NextValue()),
                    ["PacketsOutboundErrors"] = Convert.ToUInt32(instance.Value["Packets Outbound Errors"].NextValue()),
                    ["PacketsReceivedDiscarded"] = Convert.ToUInt32(instance.Value["Packets Received Discarded"].NextValue()),
                    ["PacketsReceivedErrors"] = Convert.ToUInt32(instance.Value["Packets Received Errors"].NextValue()),
                    ["OutputQueueLength"] = Convert.ToUInt32(instance.Value["Output Queue Length"].NextValue()),
                };
                items[index++] = item;
            }
